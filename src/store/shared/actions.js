...
const saveToCart= await fetch('https://indiarush.com/irapi/cart/productAddToCart?itemId=""&product='+item.product_id+'&quote_id='+quoteId+'&isAjax='+"1"+'&current_p_id=""'+'&product_quantity='+item.quantity)
.then((result) => {
	return result.json();
}).then((jsonResult) => {
	return jsonResult;
