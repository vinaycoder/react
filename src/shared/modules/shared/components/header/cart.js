		...
		<div className={' ' + (currentPage ? '' : '')}>
			<div className="columns is-mobile">
				...
				<div className="column cartPopTop">
					...
					<div className="save-for-later-mrp is-inline mrLR2">
						{' '}
						{helper.formatCurrency(item.mrpprice, settings)}
					</div>
					<div className="save-for-later-offer-price is-inline mrLR2">
						{' '}
					...
					{item.isConfigurableProduct && (
						<div className="cartPopSize mrLR2">
							...
							<span id={'size_' + item.itemId}>
								...
								&nbsp;&nbsp;
								<span
									style={{ color: '#f6823c', cursor: 'pointer' }}
									onClick={e => showSize(e, item.id, item.itemId)}
								>
									Edit
								</span>
							</span>
							&nbsp;&nbsp;
							<span
								id={'sizeShow_' + item.itemId}
								style={{ display: 'none' }}
								className="select is-small mrL5"
							>
								<select
									onChange={e => {
					...
					{item.deliveryDate == '' ? (
						<div>
							<div
								className="shipping-days-cartPop mrLR2"
								id={'shippingDate' + item.itemId}
							>
								Ships in 7 business days.
							</div>
							<div
								className="delivery-days-cart-popup mrLR2"
								id={'checkpincode' + item.itemId}
								onClick={e => showPincode(e, item.id, item.itemId)}
							>
								Get delivery dates
							...
							<div
								className="cash-delivery-cartpopup mrLR2"
								id={'cod' + item.itemId}
							>
			...
			<div className="saveLaterBtns">
				<div className="cart-buttons">
					<div
						className="new-cart-save-for-later"
						onClick={e => {
					...
					<div
						className="new-cart-remove right"
...
export default class Cart extends React.PureComponent {
	...
	showSize(e, product_id, item_id) {
		...
		document.getElementById('sizeShow_' + item_id).style.display =
			'inline-block';
	}
				...
				.then(jsonResult => {
					...
					if (jsonResult.metadata.message == 'success') {
						if (jsonResult.data.postpaid) {
							document.getElementById('cod' + item_id).innerHTML =
								'Cash On Delivery Available.';
						} else {
							document.getElementById('cod' + item_id).innerHTML =
								'Cash On Not Delivery Available.';
						}
						document.getElementById('shippingDate' + item_id).innerHTML =
							'Get it by : ' + jsonResult.data.deliveryDate;
						document.getElementById('checkpincode' + item_id).style.display =
							'none';
		...
		if (cart && cart.items && cart.items.length > 0) {
			...
			return (
				<div className="mini-cart cartPopBottom">
					<p className="save-for-later-counts is-inline">
						{items.length} Items
					</p>
					<p className="save-for-later-counts is-inline is-pulled-right">
						Total :{' '}
						<span className="cartPopTopPrice">
							{' '}
					...
					<div className="cart-main-div">{items}</div>
						...
						<div className="column is-12 cartPopBottom">
							{cart.discount == 0 && (
								<p className="checkout-coupon-text" onClick={this.toggleMenu}>
									Have a coupon code?{' '}
									<span style={{ color: '#f6701d', cursor: 'pointer' }}>
										Click Here{' '}
									</span>
								</p>
							)}
								...
								<div className="cartCouponSection">
									{cart.discount > 0 ? (
										<div className="appliedCouponMar">
											<p className="couponDiscountLable">
												Subtotal :{' '}
												<span className="cartPopTopPrice">
													{' '}
											...
											<p className="couponDiscountLable couponDiscountLableLastChild">
												Total :{' '}
												<span className="cartPopTopPrice">
													{' '}
													{helper.formatCurrency(cart.grandtotal, settings)}
											...
											<div className="appliedCouponCode">
												<input
													...
													className="coupon_code input-text"
													id="coupon_code"
													name="coupon_code"
													value={'' + cart.applied_coupon}
													readOnly
												/>
												<span
							...
							<p className="item-in-your">
								{' '}
								Items in your cart are not reserved and can sell out.{' '}
							</p>
							<NavLink
